<template>
  <v-container>
    <h2 class="text-h5 text-center nm-3 mt-5">Classificação</h2>
    <v-simple-table>
      <template v-slot:default>
        ...
        <tbody>
          <tr v-for="(clube, index) of clubesListaOrd" :key="clube.id">
            <td>{{ index + 1 }}</td>
            <td>
              <v-avatar size="24">
                <img :src="clube.escudo" :atl="clube.nome" />
              </v-avatar>
...
export default {
  ...
  computed: {
    clubesListaOrd() {
      const listaOrdenanda = this.clubesLista
        .slice(0)
